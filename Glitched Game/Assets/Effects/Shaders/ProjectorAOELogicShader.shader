...
Shader "Shader Forge/ProjectorAOELogicShader" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _node_4443; uniform float4 _node_4443_ST;
            ...
            float4 frag(VertexOutput i) : COLOR {
                float4 node_17 = tex2D(_node_4443,TRANSFORM_TEX(i.uv0, _node_4443));
                float4 _OpacityClipMask_var = tex2D(_OpacityClipMask,TRANSFORM_TEX(i.uv0, _OpacityClipMask));
                float3 node_9610 = (node_17.rgb*i.vertexColor.rgb*_TempValue*_node_8850*(1.0 - ceil((_OpacityClipMask_var.r+(_DissolveAmount*1.0009+-1.0)))));
                clip(dot(node_9610,float3(0.3,0.59,0.11)) - 0.5);
////// Lighting:
////// Emissive:
